...
La libreria **Lingui** è configurata per individuare ed estrarre automaticamente le stringhe da tradurre esclusivamente all'interno di percorsi specifici del progetto. In particolare, il processo di localizzazione analizza solo le cartelle: **`nextjs/app`**, **`nextjs/types`** e **`nextjs/components`**.
 All'interno di queste directory si trovano i componenti, i file di tipo e le parti applicative che contengono le label e i testi soggetti a localizzazione. Questo approccio garantisce un'estrazione mirata, evitando di includere file o sezioni del progetto che non richiedono le traduzioni.

Lo sviluppatore crea le nuove componenti con le stringhe/label da tradurre. Le  API della libreria Lingui impongono una  sintassi diversa nelle componenti client e server. 
...
import { Request } from "@/prisma/client/client";

const prisma = PrismaConnection.client;

/**
 * Determines whether something exists.
 * @param ts
 */
function $exists<T>(ts: T[]): boolean {
  return ts.length > 0
}

export async function usernameExists(username: string): Promise<Result<boolean>> {
 ...
 try {
   ...
   return Err(i18n._(t`Non sono riuscito a recuperare le richieste`));
 }
}
...
import { withResult } from "@/libs/server-action"; // Le server actions di NextJs nascondono gli errori lato server. L'utilizzo di Ok(), Err() e withResult() ha lo scopo di mostrare gli errori all'utente lato UI
import { usernameExists } from "./actions";
...
export default function NewUser() {
  ...
  // ...
  const checkUsernameExists = useEffectEvent(async () => {
    if (username.valid) {
      const exists = withResult(await usernameExists(username.value));
      if (exists) {
        setUsername({
          valid: false,
  ...
  // Check existence when values change
  useEffect(() => {
    checkUsernameExists();
  }, [username]);
  ...
  return (
    ...
    <FormControl
      ...
      >
        <InputLabel htmlFor="username">{t`Nome dell'utente`}</InputLabel>
        <Input
          required
          id="username"
          name="username"
